...
stats_short = stats_short.assign(
    success=(stats_short['tstat'].round(2) >= 1.96).astype(int)
)[['signalname', 'success', 'tstat', 'rbar']]

stats_ls_insamp = pd.read_excel(
    f"{pathDataPortfolios}/PlaceboSummary.xlsx", 
    sheet_name='ls_insamp_only'
)
stats_ls_insamp = stats_ls_insamp.assign(
    success=(stats_ls_insamp['tstat'].round(2) >= 1.96).astype(int)
)[['signalname', 'success', 'tstat', 'rbar']]

statsFull = pd.concat([stats_short, stats_ls_insamp], ignore_index=True)
